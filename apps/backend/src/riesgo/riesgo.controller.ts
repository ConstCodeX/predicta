...
export class RiesgoController {
  ...
  private seirMock(
    ...
    return {
      ...
      explicacion_pronostico: `El modelo tomó como base los registros históricos de dengue de ${regionKey} del sistema MINSA-CDC (${casosBaseAnual.toLocaleString()} casos/año en año típico, pico de ${casosPeakAnual.toLocaleString()} casos en el peor año registrado). El escenario ENOS "${intensidad}" ${intensidad === 'fuerte' ? 'amplificó la proyección por encima del peor año histórico' : intensidad === 'moderado' ? 'elevó la proyección a un punto intermedio entre el año base y el pico histórico' : 'mantuvo la proyección en niveles del año base'}, con un ajuste adicional por la anomalía de lluvias (${params?.anomalia_lluvias_pct ?? 0}%) y la eficiencia de control vectorial (${params?.eficiencia_control_vectorial_pct ?? 80}%). La curva epidémica proyecta un pico en la semana ${picoSemana} con ${maxSemanal.toLocaleString()} casos semanales máximos y un Rt de ${rtMax}, lo que representa una saturación hospitalaria del ${Math.min(satHospitalaria, 250)}% sobre las ${camas} camas disponibles. El nivel de riesgo ${nivelRiesgo.toUpperCase()} ${nivelRiesgo === 'critico' ? 'requiere activación inmediata del Plan de Contingencia DIRESA y coordinación con MINSA central' : nivelRiesgo === 'moderado' ? 'exige refuerzo de vigilancia epidemiológica y campañas de descacharrización' : 'es manejable con las acciones de vigilancia rutinaria del sistema de salud'}.`,
      generado_por: 'agente:dengue-seir',
    };
  }
}
